...
stopLoss = float(0)
levMargSymbol = f"{symbolName}USDT"
#leverageControl = exchange.fapiPrivate_post_leverage({"symbol": levMargSymbol,"leverage": leverage,})
#time.sleep(5)
#marginSideControl = exchange.fapiPrivatePostMarginType ({"symbol": levMargSymbol,"marginType": 'CROSSED',})
...
while True:
    try:
        ...
        controllerActiveCounter = 60
        while True:
            services.execute(f"SELECT symBil, CASE WHEN coinStatus = 'RUNNING' THEN 'TRUE' WHEN coinStatus = 'STOPPED' THEN 'FALSE' WHEN coinStatus = 'FORCED' THEN 'FORCE' END STATUSCOIN FROM coinRunner WHERE symBil = '{symbolName}' ")
            coinActive=services.fetchall()
            for activeted in coinActive:
                controllerActive = f"{activeted[1]}"
            connection.commit()
            if controllerActive == "FORCE":
                longForcedExit()
                shortForcedExit()
                sys.exit(0)
            ...
            if controllerActive == "FALSE":
                print(f"Coin; {symbolName} Will be close ({controllerActiveCounter})minute after !")
                controllerActiveCounter -= 1
                time.sleep(60)
                if controllerActiveCounter == 0:
                    services.execute(f"UPDATE coinRunner SET coinStatus = 'CLOSED' WHERE coinStatus = 'STOPPED' AND symBil = '{symbolName}' ")
                    longForcedExit()
                    shortForcedExit()
                    connection.commit()
                    sys.exit(0)
        ...
        #Forced EXIT
        def longForcedExit():
            order = exchange.create_order(symbol=symbol, amount=float(position_info["positionAmt"][len(position_info.index) - 1]), side='SELL', type='MARKET', params={'positionSide': 'LONG'})
        def shortForcedExit():
            order = exchange.create_order(symbol=symbol, amount=(float(position_info["positionAmt"][len(position_info.index) - 1]) * -1), side='BUY', type='MARKET', params={'positionSide': 'SHORT'})
        ...
        for x in LongCoins:
            ...
            if float(exractCoin[1]) < currentPrice:
                ...
                services.execute(f"INSERT INTO orderHistory VALUES (null, '{newSymbol}', '{exractCoin[4]}', '{exractCoin[1]}', 'LONG', '{exractCoin[2]}', '0.5', '{current_time}','1')")
        ...
        for y in ShortCoins:
            ...
            if float(exractCoin[1]) > currentPrice:
                ...
                services.execute(f"INSERT INTO orderHistory VALUES (null, '{newSymbol}', '{exractCoin[4]}', '{exractCoin[1]}', 'SHORT', '{exractCoin[2]}', '0.5', '{current_time}','1')")
                services.execute(f"UPDATE activeOrder SET active = '0' WHERE tid = '{exractCoin[0]}'")
                setYeniMaxMargin= float(TOPUSDT[0]) + 0.50
                services.execute(f"UPDATE toplamValletUSDT SET valueMaxMargin = '{setYeniMaxMargin}' WHERE tid = '1'")
                connection.commit()
